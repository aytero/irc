...
OperCmd::OperCmd(bool auth, Server *server) : Command(auth, server) {}

void OperCmd::execute(Client *client, std::vector <std::string> args) {
	if (args.size() < 2) {
...
//		of the new operator by issuing a "MODE +o" for the clients nickname
		...
		client->addReply(server_->getHostname(), RPL_YOUREOPER(client->getNickname()));
		// MODE +o
